...
最终方案主要是解决平台构建时因不会 `git clone` 导致不能获取文件的最近更新时间。方法很简单，每次 git commit 之后把 `md/mdx` 文件最近更新时间写入 JSON 文件，并提交 git 即可。

参考脚本如下（代码位于 `./tasks/update-post-timestamps.js`）：
...
(async function() {
  ...
  for (const file of files) {
    ...
    let updatedAt = stdout.trim();
    // Read from file mtime if file first add to git
    if (!updatedAt) {
      const stat = await fs.stat(file);
      updatedAt = moment
      .tz(stat.mtime, 'Asia/Shanghai')
      .format();
    }
    result[file] = {
      updatedAt: updatedAt,
...
平台构建时，从 JSON 读取最近更新时间，参考 Gatsby 文档在 `gatsby-node.js` 添加 Gataby Node Field 即可。
