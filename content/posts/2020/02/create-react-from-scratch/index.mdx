...
熟悉 React 的同学都知道，我们直接在组件渲染的时候返回一段类似 html 模版的结构，这个就是所谓的 JSX。JSX 本质上还是 JS，是语法糖而不是 html 模版，一般需要 [babel](https://babeljs.io/) 转换成调用 `React.createElement`，函数入参如下：
